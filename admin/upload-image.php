<?php 
$target_dir = "postimages/".date('Y-m-d')."-";
...
// Check file size
if ($_FILES["postimage"]["size"] > 800000) {
...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["postimage"]["tmp_name"], $target_file)) {
    echo '<script>The file ". htmlspecialchars( basename( $_FILES["postimage"]["name"])). " has been uploaded.</script>';
