...
if(isset($_POST['searchtitle'])){
    $_POST['search'] = filter_input(INPUT_POST, 'searchtitle', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
}
...
$query = "SELECT p.id as pid, p.post_title as posttitle, p.posts_image as postimage, p.content as postcontent, p.created_at as postingdate, c.id as cid, c.title as category
            ...
            WHERE p.post_title LIKE CONCAT('%', :searchtitle, '%')
            ORDER BY p.id DESC
            LIMIT $offset, $no_of_records_per_page";
$statement1 = $db->prepare($query);
$statement1->bindValue(':searchtitle', $_POST['searchtitle']);
$statement1->execute();
